...
import { sanitizeAvatar } from "@/lib/utils";
    ...
    const fetchAllUsers = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const data = Array.isArray(responseData) ? responseData : responseData.users || [];
          setAllUsers(
            data.map(
              (
                u: {
          ...
          const mappedChats = data.map(
            (
              ...
              return {
                ...
                avatar:
                  chat.type === "private"
                    ? sanitizeAvatar(otherParticipant?.avatar)
                    : sanitizeAvatar(chat.avatar),
                type: chat.type,
