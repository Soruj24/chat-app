...
const io = new Server(server, {
  cors: {
    origin: [
      "https://chat-app-nine-neon-38.vercel.app",
      "http://localhost:3000",
      "http://127.0.0.1:3000",
      "http://localhost:3001",
      "http://127.0.0.1:3001",
...
io.on("connection", (socket) => {
  ...
  socket.on("send_message", ({ chatId, message, receiverId }) => {
    console.log(
      `Message from ${message.senderId} to ${receiverId || "group"} in chat ${chatId}`,
    );
  ...
  socket.on("join_chat", (chatId) => {
    ...
    if (userId) {
      socket
        .to(chatId)
        .emit("user_status_update", { userId, status: "online" });
    }
  });
  ...
  socket.on("message_reaction", ({ chatId, messageId, reactions, userId }) => {
    console.log(
      `Reaction from ${userId} to message ${messageId} in chat ${chatId}`,
    );
    socket
      .to(chatId)
      .emit("message_reaction", { messageId, reactions, userId });
  });

  socket.on("message_pin", ({ chatId, messageId, isPinned }) => {
    console.log(
      `Pin update in chat ${chatId}: message ${messageId} isPinned=${isPinned}`,
    );
  ...
  socket.on("new_chat", ({ chat, participants }) => {
    console.log(
      `New chat created: ${chat.id || chat._id} with ${participants?.length} participants`,
    );
