...
import { UserStatus } from "../models/interfaces/IUser";
...
export const validateUserStatus = (user: IUser): void => {
  ...
  if (userStatus === "inactive" || userStatus === "suspended" || userStatus === "deleted") {
    throw createError(
      403,
...
export const autoActivateUser = async (user: IUser): Promise<void> => {
  const userStatus = (user as any).status;
  if (userStatus === "pending" || userStatus === "active") {
    await User.findByIdAndUpdate(user._id, { status: UserStatus.ACTIVE });
  }
};
